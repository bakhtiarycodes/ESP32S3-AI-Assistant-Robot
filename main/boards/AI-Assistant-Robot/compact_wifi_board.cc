...
private:
    ...
    void InitializeTools() {
        ...
        mcp_server.AddTool("self.motor.move_forward",
            ...
            PropertyList({
                ...
                Property("duration_ms", kPropertyTypeInteger, 5000, 100, 5000)
            }),
        ...
        mcp_server.AddTool("self.motor.turn_left",
            ...
            "Args:\n"
            "  `speed_percent`: Motor speed (0-100), default 100\n"
            "  `duration_ms`: Turn duration in milliseconds, default 5000\n"
            "Return:\n"
            "  Success message with parameters",
            PropertyList({
                Property("speed_percent", kPropertyTypeInteger, 100, 0, 100),
    ...
    void MotorTurnLeft(int duration_ms = 5000, uint8_t speed_percent = 100) {
        HandleMotorActionForApplication(3, speed_percent, duration_ms, 2); // 3 = left, high priority
    }

    void MotorTurnRight(int duration_ms = 5000, uint8_t speed_percent = 100) {
        HandleMotorActionForApplication(1, speed_percent, duration_ms, 2); // 1 = right, high priority
    }

    // Alias for compatibility
    void MotorTurnLeftDuration(int duration_ms = 5000, uint8_t speed_percent = 100) {
        MotorTurnLeft(duration_ms, speed_percent);
    }

    void MotorTurnRightDuration(int duration_ms = 5000, uint8_t speed_percent = 100) {
