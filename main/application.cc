...
// Motor control functions - only available on the AI-Assistant-Robot board
    ...
    if (new_state != last_state_for_motor) {
        switch (new_state) {
            case kDeviceStateListening:
                ESP_LOGI("Application", "状态变化事件: 唤醒 - 不执行电机动作");
                break;
            case kDeviceStateSpeaking:
                ESP_LOGI("Application", "状态变化事件: 开始说话 - 不执行电机动作");
                break;
            default:
...
void Application::TriggerMotorEmotion(int emotion_type) {
    ...
    switch (emotion_type) {
        ...
        case 7: // 唤醒反馈：前进后退平衡（最高优先级）
            ...
            break;
        case 8: // 开始说话反馈已禁用，避免说话前的短暂移动
        case 9: // 结束说话反馈已禁用，避免说话后的短暂移动
            ESP_LOGI("Application", "说话电机反馈已禁用");
            break;
        default:
...
void Application::HandleMotorActionWithDuration(int direction, int speed, int duration_ms, int priority) {
    ...
    const uint32_t action_generation = motor_action_generation_.fetch_add(1) + 1;
    ...
    if (duration_ms > 0) {
        ...
        auto stop_task = [](void* params) -> void {
            auto* timing_params = static_cast<std::tuple<int, int, Application*, uint32_t>*>(params);
            auto [duration, priority, app, generation] = *timing_params;
            delete timing_params;

            vTaskDelay(pdMS_TO_TICKS(duration));
            if (app->motor_action_generation_.load() == generation &&
                app->current_motor_priority_.load() == priority) {
                app->StopRealtimeMotorControl();
            }
            vTaskDelete(NULL);
        };

        auto* params = new std::tuple<int, int, Application*, uint32_t>(duration_ms, priority, this, action_generation);
...
void Application::HandleWebMotorControl(int direction, int speed) {
    ...
    switch (direction) {
        case 1:
            duration_ms = motor_action_config_.right_turn_duration_ms;
            break;
        case 2:
            duration_ms = motor_action_config_.backward_duration_ms;
            break;
        case 3:
            duration_ms = motor_action_config_.left_turn_duration_ms;
            break;
        case 4:
            duration_ms = motor_action_config_.forward_duration_ms;
            break;
        default:
            ESP_LOGW(TAG, "Unknown web motor direction: %d", direction);
            return;
    }

    HandleMotorActionWithDuration(direction, speed, duration_ms, 1);
}
...
void Application::SetRealtimeMotorCommand(int direction, int speed) {
    ...
    if (motor_pwm_initialized_member_) {
        ...
        // NOTE: The physical robot wiring is mirrored relative to the logical left/right names,
        // so the wheel pair assignment must be corrected here to match the commanded direction.
        switch (direction) {
            case 1: // 右: LB (ch1) + RF (ch2)
...
void Application::InitMotorPwm() {
    ...
    // Install fade service for smooth transitions only once. Repeated installs on the same
    // LEDC unit may return ESP_ERR_INVALID_STATE and should not be treated as a hard failure.
    esp_err_t fade_err = ledc_fade_func_install(0);
    if (fade_err != ESP_OK && fade_err != ESP_ERR_INVALID_STATE) {
        ESP_LOGE(TAG, "InitMotorPwm: ledc_fade_func_install failed: %s", esp_err_to_name(fade_err));
    }
...
void Application::LoadMotorActionConfig() {
    ...
    motor_action_config_.left_turn_duration_ms = settings.GetInt("left_turn_ms", 5000);
    motor_action_config_.right_turn_duration_ms = settings.GetInt("right_turn_ms", 5000);
    bool migrated_legacy_defaults = false;
    if (motor_action_config_.forward_duration_ms == 20000) {
        motor_action_config_.forward_duration_ms = 5000;
        migrated_legacy_defaults = true;
    }
    if (motor_action_config_.backward_duration_ms == 20000) {
        motor_action_config_.backward_duration_ms = 5000;
        migrated_legacy_defaults = true;
    }
    if (motor_action_config_.left_turn_duration_ms == 20000) {
        motor_action_config_.left_turn_duration_ms = 5000;
        migrated_legacy_defaults = true;
    }
    if (motor_action_config_.right_turn_duration_ms == 20000) {
        motor_action_config_.right_turn_duration_ms = 5000;
        migrated_legacy_defaults = true;
    }
    ...
    motor_action_config_.quick_forward_duration_ms = settings.GetInt("quick_fwd_ms", 10000);
    motor_action_config_.quick_backward_duration_ms = settings.GetInt("quick_bwd_ms", 10000);
