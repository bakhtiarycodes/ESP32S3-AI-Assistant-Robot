...
GpioLed::GpioLed(gpio_num_t gpio, int output_invert, ledc_timer_t timer_num, ledc_channel_t channel) {
    ...
    // Initialize fade service only once. The rest of the app may also use LEDC fade,
    // so a duplicate install is not fatal but it can trigger invalid state issues.
    esp_err_t fade_err = ledc_fade_func_install(0);
    if (fade_err != ESP_OK && fade_err != ESP_ERR_INVALID_STATE) {
        ESP_LOGE(TAG, "Failed to install LEDC fade service: %s", esp_err_to_name(fade_err));
    }
